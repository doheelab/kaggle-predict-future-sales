...
# 10001 이상의 값들은 outlier에 해당
plt.figure(figsize=(10, 4))
plt.xlim(-100, 3000)
sns.boxplot(x=train.item_cnt_day)

# 100000 이상의 값들은 outlier에 해당
plt.figure(figsize=(10, 4))
...
# On the other hand, train set contains only pairs which were sold or returned in the past.
...
# Aggregate train set by shop/item pairs to calculate target aggreagates, then <b>clip(0,20)</b> target value.
# This way train target will be similar to the test predictions.
# <i>I use floats instead of ints for item_cnt_month to avoid downcasting it after concatination with the test set later.
# If it would be int16, after concatination with NaN values it becomes int64, but foat16 becomes float16 even with NaNs.</i>
